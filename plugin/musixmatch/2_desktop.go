...
func fetchLyricsFromDesktopAPI(input lyrics.GetLyricsRequest) (lyrics.GetLyricsResponse, utils.LyricsFormat, error, *utils.LookupFailure, *utils.LookupSuccess) {
	...
	if resp, ok := lyricsFromRichsync(body.MacroCalls["track.richsync.get"]); ok && lyricsResponseAllowed(resp) {
		...
		return resp, utils.LyricsFormatSyncedLRC, nil, nil, success
	} else if ok {
	...
	if resp, ok := lyricsFromPlain(body.MacroCalls["track.lyrics.get"]); ok && lyricsResponseAllowed(resp) {
		...
		return resp, utils.LyricsFormatPlain, nil, nil, success
	} else if ok {
		utils.LogInfof("desktop_api: rejected lyrics reason=generated_pseudo_lyrics")
	}
