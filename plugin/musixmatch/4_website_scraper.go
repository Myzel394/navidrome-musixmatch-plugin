	...
	if err == nil {
		if len(tracks) > 0 {
			...
			for _, track := range tracks {
				...
				resp, format, err, failure, success := scrapeWebsiteLyricsForTrack(track, input)
				if isIdentityRejection(err) {
					utils.LogInfof("FetchLyrics: website candidate rejected reason=%s", err.Error())
					continue
				}
				if err != nil {
					return resp, format, err, failure, success
				}
				if len(resp.Lyrics) > 0 {
					...
					return resp, format, nil, nil, success
				}
			}

			if lastFailure != nil {
				return lyrics.GetLyricsResponse{}, utils.LyricsFormatUnknown, fmt.Errorf("website search failed"), lastFailure, nil
			} else {
				return lyrics.GetLyricsResponse{}, utils.LyricsFormatUnknown, fmt.Errorf("website search found no lyrics"), nil, nil
			}
		} else {
			return lyrics.GetLyricsResponse{}, utils.LyricsFormatUnknown, fmt.Errorf("website search found no tracks"), nil, nil
		}
	} else {
		status := 0
		reason := ""
...
func scrapeWebsiteLyricsForTrack(track *Song, input lyrics.GetLyricsRequest) (lyrics.GetLyricsResponse, utils.LyricsFormat, error, *utils.LookupFailure, *utils.LookupSuccess) {
	...
	if len(trackData.TrackStructureList) > 0 {
		...
		if lrc != "" {
			...
			}, utils.LyricsFormatSyncedLRC, nil, nil, success
		}
	}
	...
	}, utils.LyricsFormatPlain, nil, nil, success
}
