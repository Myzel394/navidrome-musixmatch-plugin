...
- `plugin/manifest.json` defines Navidrome plugin metadata, HTTP permissions for `apic-desktop.musixmatch.com` and `www.musixmatch.com`, and plugin config schema.
...
The desktop API path does not require user-entered Musixmatch cookies or an official API key. It automatically fetches an anonymous desktop `user_token` and caches it under `musixmatch_desktop_user_token` for 600 seconds.
